...
public boolean hasPassedQuiz(Student student, Quiz quiz,String courseId) {
    if (student == null || quiz == null) return false;

    // نجيب الـ Progress للكورس الخاص بالـ Quiz
    progress p = student.getProgressForCourse(quiz.getCourseId()); // لازم Quiz يعرف CourseId أو Lesson → Course
    if (p == null) return false;

    for (StudentQuizRecord record : p.getQuizRecords()) {
        if (record.getQuiz(courseId) != null 
            && record.getQuiz(courseId).getId() == quiz.getId() 
            && record.isPassed()) {
            return true;
        }
    }
